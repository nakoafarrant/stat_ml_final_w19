...
kable(top3.merge, col.names = c("PC1", "Feature", "Administrative Level"), caption = "County & Subcounty PC1 Top 3 Absolute Loadings")  %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width=FALSE) %>% scroll_box(width = "100%")
```
...
subct.opp$PC2 = round(subct.opp$PC1, 4)
...
title("Proportion of Variance Explained and Cumulative PVE at Subounty Level")
...
abline(h=0.9, col = "red",lty=2)
title("Proportion of Variance Explained and Cumulative PVE at County Level")
...
predict.logistic.train = predict(candidate_logistic, trn.cl, 
                               type = "response")
predict.logistic.test = predict(candidate_logistic, tst.cl, 
                              type = "response")

# calculate the training and test error and input values in records matrix for tree
records[2,1] = calc_error_rate(predict.logistic.train, trn.cl$candidate)
records[2,2] = calc_error_rate(predict.logistic.test, tst.cl$candidate)
...
y.tst = tst.cl$candidate

grid = 10^seq(10, -2, length = 100)

lasso.mod = glmnet(x.trn, y.trn %>% as.numeric(), alpha=1, lambda = grid)
plot(lasso.mod)

set.seed (1)
cv.out =cv.glmnet(x.trn, y.trn %>% as.numeric(),alpha =1)
plot(cv.out)
abline(v = log(cv.out$lambda.min), col="red", lwd=3, lty=2)
bestlam = cv.out$lambda.min
lasso.pred.train = predict(lasso.mod, s = bestlam, newx = x.trn)
lasso.pred.test = predict(lasso.mod, s = bestlam, newx = x.tst)

lasso.mse = mean((lasso.pred.test-y.tst)^2)

records[3,1] = calc_error_rate(lasso.pred.train, y.trn)
records[3,2] = calc_error_rate(lasso.pred.test, y.tst)
```
...
pred.tree <- prediction(predict.tree.train, trn.cl$candidate)
perf.tree <- performance(pred.tree, "tpr", "fpr")

pred.logistic = prediction(predict.logistic.test, trn.cl$candidate)
