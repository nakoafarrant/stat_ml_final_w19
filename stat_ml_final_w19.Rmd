...
author: "Nakoa Farrant, perm: 722447, PSTAT 231"
output:
...
county_winner <- election %>% 
  ...
  filter(pct == max(pct)) %>% 
  mutate(county_winner = candidate) %>% 
  select(-candidate) %>% 
  ungroup() %>% 
  arrange(fips)

state_winner_df <- election_state %>% 
  group_by(state) %>% 
  mutate(total = sum(votes)) %>% 
  mutate(pct = votes/total) %>% 
  filter(pct == max(pct)) %>% 
  mutate(state_winner = candidate) %>% 
  select(-candidate) %>% 
  select(fips, state_winner)
...
states_joined = left_join(states, state_winner_df, by = "fips")

```

```{r}
ggplot(data = states_joined) + 
  geom_polygon(aes(x = long, y = lat, fill = state_winner, group = group), show.legend = T) + 
  coord_fixed(1.3) +
  guides(fill=FALSE)  # color legend is unnecessary and takes too long
```
...
county.fips <- maps::county.fips %>% 
  arrange(fips) %>% 
  separate(polyname, c("region", "subregion"), extra = "merge")

```

```{r}
county.joined <- left_join(counties, county.fips, by = "subregion") %>% 
  mutate(fips = as.factor(fips))
```
 
```{r}
county.winner.joined <- left_join(county.joined, county_winner, by = "fips")
```

```{r}
ggplot(data = county.winner.joined) + 
  geom_polygon(aes(x = long, y = lat, fill = county_winner, group = group), show.legend = T) + 
  coord_fixed(1.3) +
  guides(fill=FALSE)  # color legend is unnecessary and takes too long
```
...
census.join = left_join(states_joined, census.minority, by = "fips")
```


```{r}
ggplot(data = census.join) + 
  geom_polygon(aes(x = long, y = lat, fill = state_winner, group = group, alpha = Minority), show.legend = T) + 
  coord_fixed(1.3) +
  guides(fill=FALSE)  # color legend is unnecessary and takes too long
```
...
census.ct <- census.subct %>% 
  mutate_at(c(3:29), funs((weight*.))) %>% 
  summarise_at(c(3:30), sum)
```
...
After running the PCA, the three features that

There are many instances in which PC1 and PC2 have opposite signs at both the subcounty and county levels. There were 33,999 cases of opposite signs in the principal components at the subcounty level, and 1606 instances of opposite signs between the first two PCs at the county level. PC1 is associated with the TotalPop variable, and PC2 is associated with the percentage of men in the given subcounty or county. Instances where these principal components have opposite signs indicate that the TotalPop and percentage of men in that region are uncorrelated.
...
subct.pc = subct.pr$rotation[,1:2] %>% 
  as.data.frame() %>% 
  mutate(County = census.subct$County) %>% 
  mutate(State = census.subct$State)
ct.pc = ct.pr$rotation[,1:2] %>% 
  as.data.frame() %>% 
  mutate(County = census.ct$County) %>% 
  mutate(State = census.ct$State)
...
```
At the county level, at least 15 principal components are required to capture 90% of the variance. At the subcounty level, 17 principal components are needed to represent 90% of the variance in the data.
...
census.ct.clusters = cutree(census.ct.hc, 10) %>% 
  as.data.frame()
...
sanmateo.census.cluster = census.ct.clusters$Cluster[which(census.ct.clusters$County=="San Mateo")]

sanmateo.census.cluster.df <- census.ct.clusters %>% 
  filter(Cluster == sanmateo.census.cluster)
...
ct.pc5.clusters = cutree(ct.pc5.hc, 10) %>% 
  as.data.frame()
...
sanmateo.pc5.cluster = ct.pc5.clusters$Cluster[which(ct.pc5.clusters$County=="San Mateo")]

sanmateo.pc5.cluster.df <- ct.pc5.clusters %>% 
  filter(Cluster == sanmateo.pc5.cluster)
...
# may need to calculate within cluster distance differently
```


how do you argue that clustering from one approach is better than another? 
within cluster variations
 If clustering is perfect, it'll only contain very similar counties. Difficult to validate. Silhouette coefficient - could be hard

can draw box plot of distances to other cluster
...
```{r}
election.tree = tree(county_winner~.,data = trn.cl)
...
records[1,1] = calc_error_rate(predict.tree.train, trn.cl$county_winner)
records[1,2] = calc_error_rate(predict.tree.test, tst.cl$county_winner)
