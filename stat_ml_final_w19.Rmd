...
subct.top3 <- as.data.frame(subct.pc) %>% 
  mutate(PC1.subct = abs(PC1)) %>% 
  select(PC1.subct) %>% 
  arrange(-PC1.subct) %>% 
  top_n(3) %>% 
  mutate(Rank = 1:3)

ct.top3 <- as.data.frame(ct.pc) %>% 
  mutate(PC1.ct = abs(PC1)) %>% 
  select(PC1.ct) %>% 
  arrange(-PC1.ct) %>% 
  top_n(3) %>% 
  mutate(Rank = 1:3)

top3.merge = merge.data.frame(subct.top3, ct.top3, by="Rank")
```

```{r}
subct.opp <- as.data.frame(subct.pc) %>% 
  filter(sign(PC1) != sign(PC2))

ct.opp <- as.data.frame(ct.pc) %>% 
  filter(sign(PC1) != sign(PC2))
```
